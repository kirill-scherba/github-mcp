...
GitHub MCP server extracted from [ai-hub](https://github.com/kirill-scherba/ai-hub) into a dedicated standalone server. Provides 26 GitHub API tools (17 REST + 9 GraphQL) via MCP protocol.
...
| 22 | `github_pull_request_get` | Get PR metadata (title, author, base/head, draft, mergeable, stats) |
| 23 | `github_pull_request_list` | List PRs with filters (state, head, base, sort) |
| 24 | `github_pull_request_get_files` | Get changed files with patch snippets |
| 25 | `github_pull_request_list_reviews` | List reviews and review comments |
| 26 | `github_pull_request_create_review` | Create a review (APPROVE/REQUEST_CHANGES/COMMENT) |
